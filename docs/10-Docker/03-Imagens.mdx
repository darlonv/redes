...
Um `Dockerfile` (por padrão, letra `D` maiúscula e sem extensão) é um arquivo texto que contém as instruções a serem executadas para a criação de uma imagem. Arquivos `Dockerfile` são lidos pelo comando `docker build`.
...
## FROM
A diretiva `FROM` indica a partir de qual imagem a nova imagem será criada.

## RUN

A diretiva `RUN` indica que um comando deverá ser executado. É comum que seja utilizado para instalação e configuração de softwares e bibliotecas.
